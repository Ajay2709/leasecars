...
module.exports.sendRequest = function(query, req, res, callback){
	User.find(query, function(err, doc){
		...
		if(err){
			...
			doc[0].friendreq = newArr;
			console.log("added new request:"+JSON.stringify(doc[0]));
...
module.exports.showRequests = function(req,res,callback){
	...
	User.findOne({_id:req.session.passport.user},function(err,doc){ 
		console.log("friend requests for: "+doc._id);
		if(err) throw err;
		var reqs=[];
		reqs=doc.friendreq;
		console.log("new friend requests : ");
		console.log(reqs);
		if(reqs.length==0) callback(res,reqs);
		var resultArr = []; 
		console.log("reqs length:"+reqs.length);
		for(req of reqs){
			console.log(req);
			User.findOne({_id:req},function(err,doc1){
				resultArr.push(doc1);
